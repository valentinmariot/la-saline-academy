...
const CardNote: FC<CardNoteProps> = ({ title, desc, href, checkBox }) => {
  const [isChecked, setIsChecked] = useState(false);

  return (
    <div
      className={`${styles.cardNote} border ${
        isChecked ? styles.cardNoteCheck : ""
      }`}
    >
      <div className={styles.cardNoteHead}>
        ...
        {checkBox && (
          <button className="purple" onClick={() => setIsChecked(!isChecked)}>
            <BasicIcon
              name={isChecked ? "checklist-square" : "unchecked-square"}
              size="s"
            />
          </button>
        )}
